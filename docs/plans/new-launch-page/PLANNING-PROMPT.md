# Project Planning Prompt — TarvaRI Alert Viewer: Coverage Grid Launch Page
...
> Prerequisites:
>
> - `combined-recommendations.md` from Discovery
> - `agent-roster.md` from Discovery
> - Access to the full agent fleet
> - A codebase to analyze (read-only during planning)
>
> **Tracking:** Markdown files only (no Jira). All progress tracked in
> `docs/plans/new-launch-page/` and `PLANNING-LOG.md`.
...
Plans Output Path:         docs/plans/new-launch-page
...
IMPORTANT: This project uses LOCAL MARKDOWN FILES for all tracking — NOT Jira.
All SOWs, reviews, logs, and status tracking live in docs/plans/new-launch-page/.
Do not attempt to create Jira tickets, reference Jira IDs, or use any Jira
integration tools. When the pipeline calls for "tickets" or "issues", write
them as markdown entries in the appropriate plan files instead.
...
Create this folder structure under docs/plans/new-launch-page:

    docs/plans/new-launch-page/
    ├── MASTER-PLAN.md
    ├── FINAL-SYNTHESIS.md
...
Create a file: docs/plans/new-launch-page/PLANNING-LOG.md
...
When a workstream has no assigned agent (marked "TBD"):
  ...
  1. Use BOTH agent-finding tools in parallel for the best match:
     a. mcp__tarvacode-agent-selector__select_best_agent — call with the
        workstream title and objective as the task description.
     b. mcp__skill-resolver__find_skills_for_task — call with the same
        task description to discover skills that may map to agents.
     Also consider:
     c. mcp__skill-resolver__resolve_skill_context — to get full context
        for a matched skill (dependencies, related skills, activation).
     d. mcp__tarvacode-agent-selector__get_agent_skills — to verify the
        recommended agent's capabilities cover the workstream's needs.
  2. If agent-selector confidence >= 0.7, use the recommended agent.
  3. If confidence < 0.7, cross-reference with skill-resolver results.
     Call mcp__tarvacode-agent-selector__list_agents and
     mcp__skill-resolver__list_agent_skills to pick the best match
     manually based on agent descriptions and skill mappings.
  4. If neither MCP tool is available, review the agent definitions
  ...
  6. Log in SOW header: "> **Agent Resolved By:** agent-selector MCP
     (confidence: X.XX)" or "> **Agent Resolved By:** skill-resolver MCP"
     or "> **Agent Resolved By:** manual selection"
...
When a missing workstream is identified during synthesis or review:
  ...
  2. Use agent-selector AND skill-resolver to recommend an agent.
...
AT SESSION START:
  1. Read docs/plans/new-launch-page/PLANNING-LOG.md
  ...
  4. Search memory MCP for prior decisions:
     mcp__memory__search_nodes with query: "TarvaRI Alert Viewer — Coverage Grid Launch Page planning"
...
Permissions:
  - Read, Write, Edit: files under docs/plans/new-launch-page
...
Constraints:
  ...
  - Do NOT create files outside docs/plans/new-launch-page.
...
1. Read Combined Recommendations (docs/plans/new-launch-page/combined-recommendations.md)
2. Read Agent Roster (docs/plans/new-launch-page/agent-roster.md)
