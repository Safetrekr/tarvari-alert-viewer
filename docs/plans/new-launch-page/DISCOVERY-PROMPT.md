# Project Discovery Prompt — TarvaRI Alert Viewer: Coverage Grid Launch Page
...
>
> **Tracking:** Markdown files only (no Jira). Progress tracked in
> `docs/plans/new-launch-page/` and `PROGRESS.md`.
...
Project Name:         TarvaRI Alert Viewer — Coverage Grid Launch Page
Codebase Path:        /Users/jessetms/Sites/Safetrekr/tarvari-alert-viewer
Output Directory:     docs/plans/new-launch-page
Input Signal(s):      docs/plans/new-launch-page/INPUT-SIGNAL.md
                      (see Section 1A below for the full input signal)
...
Also read the companion spec files in the output directory:
  - COVERAGE-DATA-SPEC.md — Supabase tables, columns, queries, raw data
...
────────────────────────────────────────────────────────────────────────────────
1A. INPUT SIGNAL (INLINE)
────────────────────────────────────────────────────────────────────────────────

Replace the old tarva-launch capsule ring (6 hardcoded district cards in a
circular layout) with a coverage-category card grid backed by live TarvaRI
intel data from Supabase.

The launch page becomes the Coverage Dashboard: a spatial overview of all
~38 TarvaRI intelligence sources grouped by category, with overview stats, a
geographic map of recent intel, and a card grid of coverage categories.

CORE PARADIGM — LAUNCH → DISTRICT (KEEP THIS):

The existing spatial drill-down pattern is:

  Launch Page (overview) → click a card → District View (deep detail)

This paradigm stays. The old capsule ring used 6 hardcoded districts
(Agent Builder, Project Room, Tarva Chat, etc.). The new grid uses
coverage categories as the drill-down targets:

  Launch Page (coverage grid) → click a category card → District View (category detail)

When a coverage category is "entered," it opens in a district with rich
detail: filtered source list, alert timeline, severity breakdown, map
zoomed to that category's geographic footprint, etc.

The spatial morph choreography (expand card → district panel) can be
adapted or simplified, but the conceptual pattern remains.

WHAT TO KEEP (DO NOT REMOVE):

  Spatial Engine:
  - SpatialViewport + SpatialCanvas — the ZUI pan/zoom engine
  - DotGrid — background dot grid layer
  - SectorGrid — faint grid with sector labels
  - Camera hooks (use-pan, use-zoom, use-fly-to, use-camera-sync)

  Ambient Effects (the visual design we love):
  - EnrichmentLayer with HaloGlow, RangeRings, MicroChronometer
  - CoordinateOverlays, ConnectionPaths
  - OrbitalReadouts, RadialGaugeCluster
  - DeepZoomDetails, EdgeFragments
  - HorizonScanLine, SessionTimecode, CalibrationMarks

  Header + Footer (fixed viewport overlays):
  - TopTelemetryBar — mission-control-style top bar (FREQ, TRACE, EPOCH, UPLINK)
  - BottomStatusStrip — status bar at bottom edge
  - Tarva logo (top-left, recently resized to 14px)

  Navigation HUD:
  - NavigationHUD container (z-40 overlay)
  - SpatialBreadcrumb — spatial navigation trail
  - Minimap — bottom-right space map showing viewport position on the canvas
  - ZoomIndicator — current zoom level display
  - ColorSchemeSwitcher (top-right)
  - Logout button (bottom-left, above the BottomStatusStrip)

  Data Panels (push further into the space, left/right):
  - SystemStatusPanel — system health readout
  - FeedPanel — activity feed
  - SignalPulseMonitor — signal monitoring
  - ActivityTicker — recent events ticker
  These 4 panels currently sit near center. Push them further out left/right
  to make room for the new coverage card grid in the center.

  Other:
  - CommandPalette (Cmd+K)
  - EvidenceLedgerDistrict (NW quadrant)
  - Phase3Effects (narration cycle, attention choreography, enrichment cycle)
  - All CSS: atrium.css, constellation.css, enrichment.css
  - Keyboard shortcuts (Home, Cmd+K, Escape)

WHAT TO REMOVE / REPLACE:

  Remove — Capsule Ring:
  - MorphOrchestrator and its children (CapsuleRing, DistrictCapsule,
    DetailPanel, ConnectorLines, HubCenterGlyph, ConstellationView,
    DistrictBeacon)
  - MOCK_CAPSULE_DATA usage (the 6 hardcoded districts)
  - morph-ambient-fade and morph-panels-scatter CSS wrappers (these
    blur/scatter ambient elements during card morph)

  Remove or Adapt — Morph State Machine:
  - The current morph state in ui.store.ts drives the capsule ring
    expand/collapse. May need to be adapted for the new grid→district
    drill-down, or replaced with a simpler mechanism.
  - DistrictViewOverlay — currently gates on morph phase. Needs to be
    re-wired for the new drill-down flow.
  - useInitialDistrictFromUrl — loads a district from ?district= URL
    param. Keep the concept (deep-linkable categories) but update for
    coverage categories.

NEW COMPONENTS TO BUILD:

  Coverage Card Grid (center of the spatial canvas):
  - Grid of cards, one per coverage category (weather, seismic, health, etc.)
  - Each card: category name, source count, active sources, geographic
    regions (first 3 + "+N more")
  - Click card → enter category district (drill-down)
  - Click same card again or Escape → return to grid
  - Cards use the existing severity/category color system from CLAUDE.md
  - Layout: responsive grid, likely 3-4 columns, at world-space origin

  Overview Stats (above the grid):
  - 3 metric cards: Total Sources, Active Sources, Categories Covered
  - Positioned above the card grid in world-space

  Category District View (drill-down):
  - Filtered source table (sources in that category)
  - Alert timeline / recent intel for that category
  - Severity breakdown
  - Map zoomed to that category's geographic footprint
  - Source health indicators

  Geographic Map:
  - Pins for intel_normalized items with geo data
  - Colored/styled by severity or category
  - Filters when a category is selected

VISUAL DESIGN:

  Continue the existing Oblivion-inspired mission-control aesthetic:
  - Dark backgrounds with faint mono labels
  - Glass-material cards with subtle borders (border-white/[0.06])
  - Monospace fonts for data readouts
  - Accent color: rgba(14, 165, 233, ...) (sky-500)
  - Ambient effects: dot grid, halo glow, scan lines, range rings
  - Category colors per COVERAGE-DATA-SPEC.md and CLAUDE.md conventions
  - The card grid should feel part of the spatial world, not a flat web
    page dropped into the ZUI. Same glass treatment as existing capsules
    but in a grid layout instead of a ring.

REFERENCE — ARCHIVED PAGE:

  The original launch page (with capsule ring) is archived at:
    src/app/(launch)/page.archived.tsx
  Use this for reference on how the spatial engine is composed, how the
  NavigationHUD/ambient layers/data panels are layered, and the morph
  orchestrator integration pattern.

DATA SOURCE:

  All data from the shared Supabase instance via client-side queries.
  See companion spec files in this directory for full details.
  Key tables:
  - intel_sources (~38 rows) — source registry with category, status, coverage
  - intel_normalized (up to 1000 rows for map) — parsed intel with geo data
  Two TanStack Query hooks:
  - useCoverageMetrics() — fetches sources, computes stats + category rollups
  - useCoverageMapData(filters?) — fetches geo data for map pins

CONSTRAINTS:

  - pnpm only (never npm)
  - No Jira — track work in markdown files only
  - GitHub Pages target — static export, no server-side features
  - No new dependencies without discussion
  - Import motion/react (never framer-motion)
  - Types in src/lib/interfaces/ or feature-local (never src/types/)
  - Supabase client-side only — no API routes needed for data fetching

SUGGESTED IMPLEMENTATION PHASES:

  Phase A: Clean Slate
  - Archive current page.tsx
  - Strip capsule ring (MorphOrchestrator) from the launch page
  - Keep all spatial engine, ambient, HUD, header/footer
  - Verify page loads with spatial canvas + ambient effects, no cards

  Phase B: Coverage Card Grid
  - Build the coverage category card grid component
  - Wire up useCoverageMetrics() hook for live data
  - Position grid at world-space origin
  - Push data panels further out left/right
  - Add overview stats above the grid

  Phase C: Category District Drill-Down
  - Adapt or replace the morph state machine for grid→district transitions
  - Build category district detail view (source table, severity breakdown)
  - Wire up filtering by category
  - Deep-linkable via URL params (?category=weather)

  Phase D: Map + Polish
  - Add geographic map (overview and/or district view)
  - Wire up useCoverageMapData() hook
  - Visual polish pass
...
MANDATORY PRODUCT OWNER CONSULTATION (STANDARD and DEEP):
  Spawn Task (subagent_type: software-product-owner):
    "I'm conducting project discovery for TarvaRI Alert Viewer — Coverage
     Grid Launch Page.
...
AGENT SELECTION RULES:
  ...
  4. Use mcp__tarvacode-agent-selector__select_best_agent, and skills-resolver mpc (if available)
     for any work area where the best agent isn't obvious use the compbiniaton of the mcp's findings
...
Produce exactly TWO files in docs/plans/new-launch-page:

╔══════════════════════════════════════════════════════════════════════════════╗
║  DELIVERABLE 1: combined-recommendations.md                                ║
╚══════════════════════════════════════════════════════════════════════════════╝

File: docs/plans/new-launch-page/combined-recommendations.md

Structure:

    # Combined Recommendations — TarvaRI Alert Viewer: Coverage Grid
...
File: docs/plans/new-launch-page/agent-roster.md

Structure:

    # Agent Roster — TarvaRI Alert Viewer: Coverage Grid
...
pattern:
  ...
  Spawn Task (subagent_type: <specialist-agent>):
    "I'm conducting project discovery for TarvaRI Alert Viewer — Coverage
     ...
     Codebase: /Users/jessetms/Sites/Safetrekr/tarvari-alert-viewer
...
FOR AGENT SELECTION:
  mcp__tarvacode-agent-selector__select_best_agent, use in conjunction with skills-resolver mpc and aggregate the finidings
...
Create a file: docs/plans/new-launch-page/DISCOVERY-LOG.md at the start of
...
AT SESSION START:
  1. Read docs/plans/new-launch-page/DISCOVERY-LOG.md
  2. Search memory MCP for prior findings:
     mcp__memory__search_nodes with query: "tarvari-alert-viewer coverage grid discovery"
...
You have READ permission on:
  - /Users/jessetms/Sites/Safetrekr/tarvari-alert-viewer (explore freely, read any file)
  - Any referenced documentation, configs, or schemas

You have WRITE permission on:
  - docs/plans/new-launch-page (for the two deliverables + DISCOVERY-LOG.md)
...
1. Read the Input Signal in Section 1A above, plus the companion spec files
   in docs/plans/new-launch-page/
