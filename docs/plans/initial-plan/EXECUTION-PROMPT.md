# Project Execution Prompt — Tarva Launch
...
> 2. **Planning** — Turn recommendations into Phase/Workstream/SOW structure (COMPLETE — PASS, all conditions resolved)
...
> - FINAL-VALIDATION-REPORT.md verdict: PASS (all 4 blocking conditions resolved)
>
> Planning artifacts:
>
> - 28 SOWs across 5 phases (~560 acceptance criteria)
> - MASTER-PLAN.md (implementation reference: dependency chains, sequencing, checklists)
> - FINAL-SYNTHESIS.md (analytical context: cross-phase insights, architecture decisions)
> - FINAL-VALIDATION-REPORT.md (validation: all issues, conditions, recommendations)
> - PLANNING-LOG.md (38 issues tracked, 18 FIXED, 20 OPEN non-blocking)

---

```

OK so i started this build phase docs/plans/initial-plan - but had to stop -
  and change computers so the plan might call for specific links to say
  @tarva/ir or Tarva Projecs or Tarva Agent gen or whatever - the projects
  which will be displayed in this project /Users/jessetarvambp1/Sites/tarva-org/tarva-launch/docs/plans/initial-plan - so please be aware of that if you need to find the local links to these types of projects just let me know, i also might not even have them pulled from git yet -so lets just work through that together 
...
Plan Directory:       docs/plans/initial-plan
Master Plan:          docs/plans/initial-plan/MASTER-PLAN.md
Final Synthesis:      docs/plans/initial-plan/FINAL-SYNTHESIS.md
...
Discovery Outputs (supplementary context):
  ...
  Tech Decisions:           docs/plans/initial-plan/tech-decisions.md
  Visual Design Spec:       docs/plans/initial-plan/VISUAL-DESIGN-SPEC.md
  System Overview:          docs/plans/initial-plan/TARVA-SYSTEM-OVERVIEW.md
...
Read discovery outputs for additional context on decisions made during discovery.

────────────────────────────────────────────────────────────────────────────────
1a. PROJECT-SPECIFIC CONTEXT
────────────────────────────────────────────────────────────────────────────────

WHAT THIS PROJECT IS:
  Tarva Launch is a spatial mission-control webapp — a ZUI (zoomable user
  interface) that monitors 6 Tarva ecosystem applications ("districts") via
  capsule cards, station panels, and AI-driven insights. Think: a dark-themed
  spatial dashboard with CSS-transform-based zoom, morph transitions, and
...
TECH STACK (non-negotiable):
  ...
  - motion/react v12+ (animations)
  - @supabase/supabase-js (data persistence)
  - @tanstack/react-query v5 (server state)
  - @tarva/ui via link:../tarva-ui-library (component library)
  - Ollama via native fetch (local AI)
  - @anthropic-ai/sdk (Claude API, server-only)
  - pnpm (package manager)

CRITICAL CONVENTIONS (enforced across all 28 SOWs):
  1. Import animations from `motion/react`, NEVER `framer-motion`
  2. Use `pnpm` CLI exclusively, NEVER `npm`
  3. Shared interface contracts in `src/lib/interfaces/`
  4. Domain types per district in `src/types/districts/{name}.ts`
  5. Ollama access via shared native-fetch client in `src/lib/ai/ollama-client.ts`
  6. Single AI feature toggle store in `src/lib/stores/ai.store.ts`
  7. Port assignments: Project Room=3005, Tarva Chat=4000, Ollama=11434, TarvaCORE=11435
  8. All OQ tables must include Owner column

REFERENCE CODEBASE:
  Agent Builder at /Users/jessetms/Sites/tarva-claude-agents-frontend/
  serves as the architectural reference. Match its patterns for providers,
  layouts, and component structure.

PHASE STRUCTURE (28 workstreams total):
  Phase 0: Tech Spike (3 WS) — scaffolding, design tokens, spatial spike
  Phase 1: Spatial Core + Login (7 WS) — spatial engine, atrium, login, keyboard, telemetry, ambient, interfaces
  Phase 2: Districts + Stations + Morph (7 WS) — morph, 4 district contents, station framework, constellation
  Phase 3: Receipts + Command Palette + AI (7 WS) — receipts, evidence, command palette, AI router, template registry, narration, attention
  Phase 4: Advanced AI + Polish (4 WS) — Claude API, exception triage, builder mode, visual polish

PLANNING VALIDATION STATUS:
  - Final verdict: PASS (all blocking conditions resolved)
  - 38 issues tracked during planning: 18 FIXED, 20 OPEN (all non-blocking)
  - 7 deviations from discovery input (all documented and justified)
  - See FINAL-VALIDATION-REPORT.md Section 5.2 for 11 RECOMMENDED fixes
...
This is a LARGE PROJECT (28 workstreams, 5 phases). Use full protocol:
  - Full pre-flight and post-flight checks via #every-time
  - Full phase gate protocol
  - Full EXECUTION-LOG.md structure
  - Context management and memory MCP usage
...
Create a file: docs/plans/initial-plan/EXECUTION-LOG.md
...
STEP 6: COMMIT
  ...
  Commit message format:
    ...
    Plan ref: docs/plans/initial-plan/phase-x-<slug>/ws-x.n-<slug>.md
...
Create a project branch before starting: tarva-launch
...
If parallel phases are needed, create phase branches:
  tarva-launch/phase-c, tarva-launch/phase-d
...
CODE QUALITY:
  ...
  - Use `pnpm` CLI exclusively, NEVER `npm`
...
1. Read docs/plans/initial-plan/EXECUTION-LOG.md
...
4. Search memory MCP for prior decisions:
   mcp__memory__search_nodes with query: "Tarva Launch"
...
You have full read/write permission on:
  - /Users/jessetms/Sites/tarva-org/tarva-launch (the project codebase — this is where code goes)
  - docs/plans/initial-plan (the plan docs — for reading SOWs and updating tracker)
...
3. Read discovery outputs (combined-recommendations.md, tech-decisions.md,
   VISUAL-DESIGN-SPEC.md, TARVA-SYSTEM-OVERVIEW.md, agent-roster.md)
